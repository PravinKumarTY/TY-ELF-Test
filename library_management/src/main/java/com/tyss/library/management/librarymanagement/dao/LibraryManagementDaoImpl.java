...
import com.tyss.library.management.librarymanagement.mailservice.MailService;
@Repository
public class LibraryManagementDaoImpl implements LibraryManagementDao{
	...
	public boolean registerUser(UserInfoDto userInfo) {
		...
		String autogenPassword="U_"+userInfo.getUserName().replace(' ','_').toLowerCase()+(new Random().nextInt(900000) + 100000);
	...
	@Override
	public boolean addBook(BookInfoDto bookDto) {
		EntityManager em = factory.createEntityManager();
		EntityTransaction et = em.getTransaction();
